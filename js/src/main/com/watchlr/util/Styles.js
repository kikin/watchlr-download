...
$.Class.extend("com.watchlr.util.Styles", {
	...
	insertCss: function(css, element, styleName) {
        ...
        if ($.browser.msie) {
            // $cwutil.Logger.debug('Is a top document: ' + (window.document == doc));
            if (window.document == doc) {
                docWindow = window;
            } else {
                $.get('iframe').each(function(index, value) {
                    // $cwutil.Logger.debug('Is an iframe document: ' + (value.document == doc));
                    // $cwutil.Logger.debug('iframe window object: ' + value);
        ...
        // $cwutil.Logger.debug('head element: ' + head);
		...
		if ($.browser.msie) {
			try {
                ...
                // $cwutil.Logger.debug('Creating style sheet using IE native function.');
				var style = doc.createStyleSheet();
				style.cssText = css;
				
			} catch(e) {
				try {
                    // $cwutil.Logger.debug('Creating style sheet using IE hack way.');
